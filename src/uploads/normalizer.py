...
def normalize_user_dataset(raw: dict[str, Any], project: dict[str, Any]) -> dict[str, Any]:
    """Normalize JSON or template CSV input into the canonical PVE dataset shape."""
    data = deepcopy(raw)
    ...
    if not isinstance(baseline, dict):
        baseline_alternative = next(
            (
                ...
                if alternative.get("status") == "baseline"
            ),
            {},
        )
        baseline = {
            "baseline_id": "BASE-UPLOAD-001",
            "alternative_id": baseline_alternative.get("alternative_id"),
        }
    ...
    recommendation.setdefault(
        "rationale",
        "User-upload placeholder. No autonomous packaging approval is granted.",
    )

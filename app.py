...
def render_home() -> None:
    ...
    st.warning("This application uses synthetic demonstration data only. It must not be treated as validated supplier, laboratory, production, engineering-trial, or commercial data.")
    dataset = load_demo()
    alternatives = dataset["packaging_alternatives"]
    project = dataset["packaging_project"]
    st.sidebar.header("Scenario Inputs")
    annual_volume = st.sidebar.number_input("Annual volume (cases)", min_value=1.0, value=float(project["annual_volume"]), step=10000.0)
    ...
    for alternative in alternatives:
        ...
        with st.sidebar.expander(f"{alternative_id} assumptions"):
            cost_adjustments[alternative_id] = st.number_input("Unit-cost adjustment (%)", min_value=-50.0, max_value=100.0, value=0.0, step=1.0, key=f"cost-{alternative_id}")
            material_adjustments[alternative_id] = st.number_input("Material-weight adjustment (%)", min_value=-50.0, max_value=100.0, value=0.0, step=1.0, key=f"material-{alternative_id}")
    inputs = ScenarioInputs(annual_volume=annual_volume, cost_adjustment_percent_by_alternative=cost_adjustments, material_adjustment_percent_by_alternative=material_adjustments)
    ...
    for alternative_id, result in scenario.alternatives.items():
        ...
        comparison_rows.append({"Alternative": alternative_id, "Name": names[alternative_id], "Unit cost": result.unit_cost, "Annual savings": result.annual_savings_vs_baseline, "Material change %": result.material_change_percent_vs_baseline, "Qualification": qualification.status, "Risk": risk.overall_level, "Risk data complete": risk.data_complete, "Recommendation": rec.status if rec else "baseline"})
    ...
    source_commit = st.text_input("Source commit or version reference", value="LOCAL-DEMO", help="Provide the Git commit or version reference represented by this export.")
    generated_at = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    package = assemble_decision_package(dataset, scenario, qualifications, risks, recommendation, source_commit=source_commit, generated_at=generated_at)
    left, right = st.columns(2)
    with left:
        st.download_button("Download machine-readable JSON", data=render_decision_package_json(package), file_name="pve_decision_package.json", mime="application/json", width="stretch")
    with right:
        st.download_button("Download human-readable report", data=render_decision_package_markdown(package), file_name="pve_decision_report.md", mime="text/markdown", width="stretch")
    st.caption(f"Read-only internal export from {SOURCE_REPOSITORY}. The final integration contract remains draft.")
    st.info("This tool does not approve packaging designs autonomously. Recommendations and exports remain subject to engineering validation and documented evidence.")

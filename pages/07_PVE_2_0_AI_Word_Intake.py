...
from src.review_comparison import CandidateReview, ReviewError, ReviewState, confirm, correct_and_confirm, intentionally_omit, reject
...
def _load_documents() -> DocumentPair | None:
    ...
    mode = st.radio("Document source", ("Built-in synthetic pair", "Upload two synthetic DOCX files"), horizontal=True)
    existing_demo, proposed_demo = synthetic_demo_documents()
    download_columns = st.columns(2)
    download_columns[0].download_button("Download existing synthetic DOCX", existing_demo, "existing_synthetic.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", width="stretch")
    download_columns[1].download_button("Download proposed synthetic DOCX", proposed_demo, "proposed_synthetic.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", width="stretch")
    try:
        if mode == "Built-in synthetic pair":
            return load_synthetic_pair()
        upload_columns = st.columns(2)
        existing = upload_columns[0].file_uploader("Existing synthetic specification", type=["docx"], key="pve2_existing_docx")
        proposed = upload_columns[1].file_uploader("Proposed synthetic specification", type=["docx"], key="pve2_proposed_docx")
        if existing is None or proposed is None:
            st.info("Upload exactly one existing and one proposed synthetic DOCX.")
            return None
        return parse_document_pair(existing.name, existing.getvalue(), proposed.name, proposed.getvalue())
...
def _document_summary(documents: DocumentPair) -> None:
    ...
    for document in (documents.existing, documents.proposed):
        rows.append({"Role": document.role.value.title(), "Filename": document.filename, "SHA-256": document.sha256, "Source blocks": len(document.blocks), "Unsupported content": len(document.unsupported_content)})
    st.dataframe(rows, width="stretch", hide_index=True)
    for document in (documents.existing, documents.proposed):
        with st.expander(f"{document.role.value.title()} parsed source blocks"):
            st.dataframe([{"Block ID": block.block_id, "Type": block.block_type.value, "Text": block.text, "Section": block.location.section_title, "Table": block.location.table_index, "Row": block.location.row_index, "Cell": block.location.cell_index} for block in document.blocks], width="stretch", hide_index=True)
...
def _apply_review_actions(reviews: tuple[CandidateReview, ...]) -> tuple[CandidateReview, ...]:
    ...
    for review in reviews:
        ...
        with st.expander(f"{candidate.document_role.value.title()} — {candidate.field_name.replace('_', ' ').title()} — {candidate.raw_value}"):
            ...
            action = st.selectbox("Review action", ("Pending", "Confirm", "Correct and confirm", "Intentionally omit", "Reject"), key=f"action:{key}")
            try:
                if action == "Confirm":
                    updated.append(confirm(review, reviewer_note="Confirmed in synthetic demonstration"))
                elif action == "Correct and confirm":
                    corrected_value: Any = st.text_input("Corrected value", str(candidate.normalized_value), key=f"value:{key}")
                    corrected_unit = st.text_input("Corrected unit", candidate.unit or "", key=f"unit:{key}") or None
                    note = st.text_input("Correction reason", "Corrected during synthetic review", key=f"note:{key}")
                    updated.append(correct_and_confirm(review, corrected_value, corrected_unit, reviewer_note=note))
                elif action == "Intentionally omit":
                    updated.append(intentionally_omit(review, reviewer_note="Omitted during synthetic review"))
                elif action == "Reject":
...
def _comparison_rows(groups) -> list[dict[str, Any]]:
    ...
    for comparison in demo_comparisons(groups):
        rows.append({"Field": comparison.field_name.replace("_", " ").title(), "Existing": comparison.existing.effective_value if comparison.existing else None, "Existing unit": comparison.existing.effective_unit if comparison.existing else None, "Proposed": comparison.proposed.effective_value if comparison.proposed else None, "Proposed unit": comparison.proposed.effective_unit if comparison.proposed else None, "Status": comparison.status.value, "Change": comparison.change, "Change %": comparison.change_percent})
    return rows
...
def main() -> None:
    ...
    st.warning("Synthetic portfolio demonstration only. No confidential documents, live AI provider, autonomous approval, engineering qualification or realized-savings claim.")
    ...
    with extraction_tab:
        ...
        st.dataframe([{"Role": review.candidate.document_role.value, "Field": review.candidate.field_name, "Raw value": review.candidate.raw_value, "Normalized value": review.candidate.normalized_value, "Unit": review.candidate.unit, "Confidence": review.candidate.confidence, "Ambiguity": ", ".join(code.value for code in review.candidate.ambiguity_codes) or "None", "Source block": review.source.block_id, "Source excerpt": review.source.excerpt} for review in reviews], width="stretch", hide_index=True)
    with review_tab:
    ...
    with comparison_tab:
        ...
        unresolved = [f"{group.document_role.value}:{group.field_name}" for group in groups if group.selected_review is None]
        if unresolved:
    ...
    with mapping_tab:
        ...
        try:
            draft, issues, valid = build_canonical_dataset_draft(project=SYNTHETIC_PROJECT, groups=groups, source_repository="pratikoperations/Packaging-Value-Engineering-Decision-Intelligence", source_commit="draft-pr-54")
            if valid:
            ...
            snapshot = build_confirmed_snapshot(project_id=SYNTHETIC_PROJECT["project_id"], documents=documents, groups=groups, canonical_dataset_draft=draft, canonical_validation_issues=issues, canonical_validation_valid=valid, parser_version=PARSER_VERSION, extraction_schema_version=EXTRACTION_SCHEMA_VERSION, alias_registry_version=ALIAS_REGISTRY_VERSION, provider_id=PROVIDER_ID)
